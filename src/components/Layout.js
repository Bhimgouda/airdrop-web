...
const getAllocationProof = async (userid) => {
  ...
  try {
    ...
    const txn = await moiContract.getAllocationProofsOf(userid, 0x00);
    return txn;
  } catch (error) {
    return [];
  }
...
export default React.forwardRef(function Layout({ children, data }, ref) {
  ...
  const getEligibility = async () => {
    ...
    let response = await fetch(
      `/api/moi?userId=${loginData.user.userID}&userName=${loginData.userName}&token=${authToken.datToken}`
    );
    ...
    const avatarsCreated = data.interactions.data.filter(function (txn) {
      try {
        ...
        return namespace.includes("Avatar") && txn.action == "Created";
      } catch (error) {
        return;
      }
    });
    ...
    const avatarsScanned = data.interactions.data.filter(function (txn) {
      try {
        let namespace = loginData.iome.utils.mDecode(txn.namespace);
        return namespace.includes("Avatar") && txn.action == "Scanned";
      } catch (error) {
        return;
      }
    });
    ...
    const appsCreated = data.interactions.data.filter(function (txn) {
      try {
        let namespace = loginData.iome.utils.mDecode(txn.namespace);
        return namespace.includes("App") && txn.action == "Created";
      } catch (error) {
        return;
      }
    });
    ...
    const appsJoined = data.interactions.data.filter(function (txn) {
      try {
        let namespace = loginData.iome.utils.mDecode(txn.namespace);
        return namespace.includes("App") && txn.action == "Joined";
      } catch (error) {
        return;
      }
    });
  ...
  return (
    <>
      <div
        className={`p-auto bg-fixed bg-no-repeat ${
          isDarkMode ? "bg-hero-section bg-moi-white-600" : "bg-main-bg bg-[#13162e]"
        }`}
      >
        <Header name={ref} />
        <div
